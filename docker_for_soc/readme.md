...
* docker build -t docker_for_soc_build .

## 发布镜像

* docker tag docker_for_soc_build wangergou2023/docker_for_soc_build
  ...
  * 这个命令是用来给 Docker 镜像打标签的。让我们详细解释一下这个命令的各个部分：
    ...
    - `docker_for_soc_build`: 这是源镜像的名称。这里假定 `docker_for_soc_build` 镜像已经存在于你的本地 Docker 镜像库中，并且使用了默认的 `latest` 标签（除非你之前为它指定了其他标签）。


    - `wangergou2023/docker_for_soc_build`: 这是目标镜像的名称和标签。更具体地说：
      - `wangergou2023/`: 这部分是 Docker Hub 的用户名或组织名。这意味着当你稍后尝试将这个镜像推送到 Docker Hub 时，它将被推送到这个用户名或组织名下。
      - `docker_for_soc_build`: 这是镜像的名称。
      - 由于没有指定标签（例如 `:v1.0` 或 `:latest`），所以默认使用 `latest` 标签。


  * 当你运行这个命令后，Docker 将在本地创建一个新的标签（实际上是一个别名）`wangergou2023/docker_for_soc_build:latest`，它指向原始的 `docker_for_soc_build:latest` 镜像。注意，这不会复制或移动任何数据；它只是为同一个镜像添加了一个新的标签。

* docker push wangergou2023/docker_for_soc_build
...
* 使用docker指令
  ...
  * docker pull wangergou2023/docker_for_soc_build 
    * 部署到本地

  * docker run -v .:/myvolume -it wangergou2023/docker_for_soc_build /bin/bash
    * 挂载当前目录到容器

* 使用 docker-compose文件

执行 `docker-compose up -d` 命令，输出信息中的 `✔ Container soc-dev-1  Started` 表示容器 `soc-dev-1` 已经成功启动。
